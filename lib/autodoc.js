...
module.exports = async function (opts) {
  ...
  const docUrl =
    "https://github.com/openmrs/openmrs-esm-core/tree/master/packages/esm-config";
  const preamble =
    "See the [openmrs-esm-config docs](" +
    docUrl +
...
function generateArrayOfObjectsDocFromSchema(
  ...
  let doc = "\n" + outerIndents + "{";
  const keys = Object.keys(arrayElements).filter(
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (isArrayOfObjectsSchema(arrayElements[key])) {
      doc += startStr + `"${key}": [`;
      doc += documentDescriptionAndValidators(arrayElements[key], indents);
      doc += generateArrayOfObjectsDocFromSchema(
        arrayElements[key],
        thisKeyPath
      );
      doc += startStr + "]" + separator;
